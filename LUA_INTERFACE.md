...
| `dwarfipelago/manual_send` | `""` or `"1"` | Lua/panel | `"1"` pauses the mod's whole check-scanning pass. Nothing is detected or queued until a send is requested, so a fort paying too much FPS for live checks can opt out |
